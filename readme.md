...
- loop ele
- loop reach tar
- dp[n[j]] with, dp[i-n[j]] without (i>=w, threshold)
...
##### pick repeated ele in arr, fill up to tar, total_num_combo
...
- loop step tar, reach n
- loop small_num, reach i
- f1 = ma(dp[j], j) with, f2 = ma(dp[i-j], i-j) without, f1\*f2
