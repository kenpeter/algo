...
- simple:
- pArr = 2D arr
- l: each cell exists in parallel universe
- l: orig board has var -> make copy introducing more vars
- l: 1. 1D seen; 2. 2D seen*
- l: 2 g_seen cross over -> connect -> flow each other
- x
- aArr = 2D arr
- l: each cell exists ....
...
- simple:
- dfs
- if out_bound || visited -> return
- l: interview (bound)
- l: only go to un-visited area
- return (l: 1. re nothing*; 2. re overall bit/every; 3. re states)
- l: travel update board state
- g[r][c] = '.'
- l: offer (avoid same)
- l: travel update board state
- 4 dir
...
- let res = 0;
- l: 1. min/max; 2. acc*
- l: connected area count as 1
- 2D loop -> dfs
- l: each cell -> start pt to travel
- if g[r][c] == 'X' -> dfs
- l: 1. dfs; 2. condi dfs*; 3. no dfs
- l: there is path through; there is path blocked
- motion:
...
- https://leetcode.com/problems/n-ary-tree-level-order-traversal/ (bfs normal travel, but with children)
- 
